...
Scaleway Private Networks can be attached to [Public Gateways](/network/public-gateways/concepts/#public-gateway). Previously, Public Gateways could provide a DHCP service to Private Networks, assigning private IP addresses to attached resources. When [VPC went into General Availability](/network/vpc/reference-content/vpc-migration/) (GA), DHCP was moved from Public Gateways to be directly built into Private Networks and intergrated with Scaleway's internal [IPAM](/network/vpc/concepts/#ipam). Public Gateways continue to act as gateways providing ingress/egress to the public Internet, as well as NAT and SSH bastion features.

This page has been preserved in the short term as legacy content, showing how you could previously **manually** configure a standalone Public Gateway (managing only ingress/egress without DHCP) via the [Scaleway CLI](https://github.com/scaleway/scaleway-cli).
</Message>
